...
Path = Path(:,chiffre:chiffre+1); % a supprimer !!!!!
...
%% Create graph utiliser biograph

% graph = digraph();
% graph = addnode(graph,length(Nodes));
% 
% for i = 1:length(Nodes)
%     graph.Nodes.Lat(i) = {Nodes(i,1)};
%     graph.Nodes.Lng(i) = {Nodes(i,2)};
%     
% end
% 
%     graph = addedge(graph, n1, n2)
%     graph.Edges.Time(e) = {value};

% save('Graph','graph','Nodes','Nodes_Perimeter','Nodes_Reseau');
